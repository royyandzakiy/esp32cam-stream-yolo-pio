...
url = 'http://192.168.228.203/cam-hi.jpg'
...
while True:
    ...
    if output_layers.ndim == 1:  # If it's a 1D array
        outputNames = [layernames[i - 1] for i in output_layers]
    else:  # If it's a scalar (older OpenCV versions)
        outputNames = [layernames[output_layers[0] - 1]]
